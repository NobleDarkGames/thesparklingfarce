...
##
## Actions handled via ItemActionMenu popup (USE, EQUIP, UNEQUIP, DROP, INFO).
## GIVE is intentionally excluded - that's Caravan-only (SF2 authentic).
...
## Current character identifiers (always hero)
...
func _on_initialized() -> void:
	...
	# Get hero character from context
...
func _refresh_for_hero() -> void:
	...
	# Update hint - NO L/R cycling (SF2 authentic: field menu is hero only)
...
func _on_equipment_button_pressed(slot_id: String, item_id: String) -> void:
	...
	# Show ItemActionMenu with EQUIPMENT_SLOT context
	focused_slot_id = slot_id
	focused_item_id = item_id
	var button: Button = equipment_buttons.get(slot_id)
	if button:
		...
		# Use EQUIPMENT_SLOT context (value 2)
...
func _on_inventory_button_pressed(index: int, item_id: String) -> void:
	...
	# Show item action menu with FIELD_MENU context (no GIVE action)
	if inventory_buttons.size() > index:
		var button: Button = inventory_buttons[index]
		var button_rect: Rect2 = button.get_global_rect()
		var menu_pos: Vector2 = Vector2(button_rect.end.x + 4, button_rect.position.y)

		# Use FIELD_MENU context (value 3) - NO GIVE action
		var ItemActionMenuClass: Script = load("res://scenes/ui/item_action_menu.gd")
		_item_action_menu.show_menu(item_id, ItemActionMenuClass.Context.FIELD_MENU, menu_pos)
...
func _apply_item_effect(item: ItemData, target: CharacterSaveData) -> Dictionary:
	## Apply consumable item effect to hero
	if not item.effect:
