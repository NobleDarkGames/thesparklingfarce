...
## Party Editor UI
## Allows browsing and editing PartyData resources
...
## Override: Create the party-specific detail form
...
## Load all available characters from active mod
	...
	if dir:
		...
		while file_name != "":
			if file_name.ends_with(".tres"):
				var character_data: CharacterData = load(character_dir.path_join(file_name))
				if character_data:
					available_characters.append(character_data)
			file_name = dir.get_next()
		dir.list_dir_end()
	else:
		push_warning("Party Editor: Could not open character directory: " + character_dir)
