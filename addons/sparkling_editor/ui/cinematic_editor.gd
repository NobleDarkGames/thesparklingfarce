...
var _command_inspector: CinematicCommandInspector
...
func _setup_inspector_panel(parent: HSplitContainer) -> void:
	...
	_command_inspector = CinematicCommandInspector.new()
	_command_inspector.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_command_inspector.param_changed.connect(_on_inspector_param_changed)
	_command_inspector.target_changed.connect(_on_inspector_target_changed)
	inspector_scroll.add_child(_command_inspector)

	# Show placeholder initially
	_command_inspector.show_placeholder()
...
func _build_inspector_for_command(index: int) -> void:
	...
	# Ensure context is ready and set up the inspector
	_build_widget_context()
	_command_inspector.set_context(_widget_context)
	_command_inspector.set_actor_ids(_get_current_actor_ids())
	_command_inspector.load_command(cmd, _get_command_color(cmd_type))




## Handle parameter changes from the inspector component
func _on_inspector_param_changed(param_name: String, new_value: Variant) -> void:
...
## Handle target changes from the inspector component
func _on_inspector_target_changed(target: String) -> void:
	if _updating_ui or selected_command_index < 0:
		return
	var commands: Array = current_cinematic_data.get("commands", [])
	if selected_command_index >= commands.size():
		return
	commands[selected_command_index]["target"] = target
	is_dirty = true
	_rebuild_command_list()
