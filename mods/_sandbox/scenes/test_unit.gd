	...
	if debug_label:
		if not _debug_visible:
			...
			if _test_unit and _test_unit.is_alive():
				debug_label.text += "Hero: HP:%d/%d\n" % [_test_unit.stats.current_hp, _test_unit.stats.max_hp]
			if _enemy_unit and _enemy_unit.is_alive():
				debug_label.text += "Goblin: HP:%d/%d\n" % [_enemy_unit.stats.current_hp, _enemy_unit.stats.max_hp]
